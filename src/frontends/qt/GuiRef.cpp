...
void GuiRef::updateAddPB()
{
	...
	bool const notsupported = bufferview()->cursor().inMathed()
			...
			&& reftype != "labelonly"
			&& (((reftype == "vref" || reftype == "vpageref") && buffer().params().xref_package != "cleveref")
			    || reftype == "ref" || reftype == "pageref" || reftype == "eqref" || reftype == "nameref"
			    || (reftype == "cpageref" && (buffer().params().xref_package == "cleveref"
			        || buffer().params().xref_package == "zref"))
			    || (reftype == "formatted" && prefixIs(buffer().params().xref_package, "prettyref")));
